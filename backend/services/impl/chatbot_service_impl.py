...
from backend.schemas.chatbot_schema import ChatbotCreateSchema, ChatbotInDBSchema, ChatbotUpdateSchema
...
class ChatbotServiceImpl(ChatbotService):
    ...
    def create(self, db: Session, chatbot_create: ChatbotCreateSchema, current_user_role_permission: UserRolePermissionSchema) -> ChatbotInDBSchema:
        if 'create_chatbot' not in current_user_role_permission.u_list_permission_name:
            ...
            return JSONResponse(status_code=400, content="Create Chatbot failed: User does not have permission to create chatbot")
        try:
            ...
            return JSONResponse(status_code=400, content="Create Chatbot failed")
    ...
    def get_all(self, db: Session, current_user_role_permission: UserRolePermissionSchema) -> Optional[List[ChatbotInDBSchema]]:
        if 'read_chatbot' not in current_user_role_permission.u_list_permission_name:
            ...
            return JSONResponse(status_code=400, content="Get all chatbot failed: User does not have permission to read chatbot")
        try:
            chatbots: List[ChatbotInDBSchema] = self.__crud_chatbot.get_multi(db=db, filter_param={"user_id": current_user_role_permission.u_id})
        except:
            logger.exception(
                f"Exception in {__name__}.{self.__class__.__name__}.get_all"
            )
            return JSONResponse(status_code=400, content="Get all chatbot failed")
        return chatbots
    ...
    def get_one(self, db: Session, chatbot_id: str, current_user_role_permission: UserRolePermissionSchema) -> Optional[ChatbotInDBSchema]:
        if 'read_chatbot' not in current_user_role_permission.u_list_permission_name:
            ...
            return JSONResponse(status_code=400, content="Get one chatbot failed: User does not have permission to read chatbot")
        try:
            chatbot_found: ChatbotInDBSchema = self.__crud_chatbot.get_one_by(db=db, filter={"id": chatbot_id})
        except:
            logger.exception(
                f"Exception in {__name__}.{self.__class__.__name__}.get_one"
            )
            return JSONResponse(status_code=400, content="Get one chatbot failed")
        
        if chatbot_found is None:
            return JSONResponse(status_code=404, content="Chatbot not found")
    ...
    def update(
        ...
        if 'update_chatbot' not in current_user_role_permission.u_list_permission_name:
            ...
            return JSONResponse(status_code=400, content="Update chatbot failed: User does not have permission to update chatbot")

        chatbot_found: ChatbotInDBSchema = self.get_one(db=db, chatbot_id=chatbot_id, current_user_role_permission=current_user_role_permission)
        
        # logger.info(f"chatbot_found: {chatbot_found.chatbot_name}")
        # logger.info(f"chatbot_found: {chatbot_found.is_public}")
            ...
            try:
                ...
                for chatbot in chatbots:
                    if chatbot.id != chatbot_found.id:
                        ...
                        self.__crud_chatbot.patch(
                            db=db,
                            db_obj=chatbot,
                            obj_in={"is_public": False}
                        )
            except:
                logger.exception(
                    f"Exception in {__name__}.{self.__class__.__name__}.update"
                )
                return JSONResponse(status_code=400, content="Update chatbot failed")
            
        try:
            # logger.info(f"chatbot_found: {chatbot_found.is_public}")
            # logger.info(f"chatbot_update: {chatbot_update.is_public}")
            
            chatbot_updated: ChatbotInDBSchema = self.__crud_chatbot.patch(
                db=db,
                db_obj=chatbot_found,
                obj_in=chatbot_update
            )
        except:
            logger.exception(
                f"Exception in {__name__}.{self.__class__.__name__}.update"
    ...
    def delete(self, db: Session, chatbot_id: str, current_user_role_permission: UserRolePermissionSchema) -> Optional[ChatbotInDBSchema]:
        if 'delete_chatbot' not in current_user_role_permission.u_list_permission_name:
            ...
            return JSONResponse(status_code=400, content="Delete chatbot failed: User does not have permission to delete chatbot")
        chatbot_found: ChatbotInDBSchema = self.get_one(db=db, chatbot_id=chatbot_id, current_user_role_permission=current_user_role_permission)
        try:
            ...
            return JSONResponse(status_code=400, content="Delete chatbot failed")
