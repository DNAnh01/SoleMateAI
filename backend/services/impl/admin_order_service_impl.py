...
class AdminOrderServiceImpl(AdminOrderService):
    ...
    def get_order_by_id(
        ...
        try:
            order = self.__crud_order.get_one_by(
                db=db,
                filter={"id": order_id}
            )
            ...
            for order_item in order_items_db:
                if order_item.order_id == order.id:
                    ...
                    brand = self.__crud_brand.get_one_by(db=db, filter={"id": shoe.brand_id})
                    size = self.__crud_size.get_one_by(db=db, filter={"id": shoe.size_id})
                    color = self.__crud_color.get_one_by(db=db, filter={"id": shoe.color_id})
                    order_items.append(
                        OrderItemOutSchema(
                            shoe=ShoeOutSchema(
                                brand=BrandCreateSchema(**brand.__dict__),
    ...
    def deliver_order(
        ...
        try:
            ...
            logger.exception(f"Exception in {__name__}.{self.__class__.__name__}.deliver_order: {str(e)}")
            return JSONResponse(
                status_code=500, 
                content={
                    "status": 500,       
