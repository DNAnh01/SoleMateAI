...
class AddressServiceImpl(AddressService):
    ...
    def add_or_update_address(
        ...
        if (
            ...
            return JSONResponse(
                status_code=400,
                content="Add or Update Address failed: User does not have permission to add or update address",
            )
        try:
            ...
            return JSONResponse(
                status_code=400,
                content="Add or Update Address failed: Unexpected error",
            )
    ...
    def get_current_shipping_address(
        ...
        if "read_address" not in current_user_role_permission.u_list_permission_name:
            ...
            return JSONResponse(
                status_code=400,
                content="Get Current Shipping Address failed: User does not have permission to read address",
            )
        try:
            ...
            if current_shipping_address is None:
                ...
                return JSONResponse(
                    status_code=400,
                    content="Get Current Shipping Address failed: No shipping address found",
                )
            ...
            return JSONResponse(
                status_code=400,
                content="Get Current Shipping Address failed: Unexpected error",
            )
