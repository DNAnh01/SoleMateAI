...
from backend.schemas.brand_schema import BrandInDBSchema, BrandCreateSchema
from backend.schemas.color_schema import ColorInDBSchema, ColorCreateSchema
from backend.schemas.size_schema import SizeInDBSchema, SizeCreateSchema
...
class ShoeServiceImpl(ShoeService):
    ...
    def create_shoe(self, db: Session, shoe: ShoeCreateSchema,current_user_role_permission: UserRolePermissionSchema) -> ShoeOutSchema:
        ...
        try:
            ...
            brand_found = self.__crud_brand.get_one_by(
                db=db,
                filter={"brand_name": shoe.brand.brand_name}
            )
            ...
            color_found = self.__crud_color.get_one_by(
                db=db,
                filter={"hex_value": shoe.color.hex_value}
            )
            ...
            size_found = self.__crud_size.get_one_by(
                db=db,
                filter={"size_number": shoe.size.size_number}
            )
            ...
            shoe_out = ShoeOutSchema(
                ...
                size=SizeCreateSchema(
                    size_number=shoe_saved.size.size_number
                ),
    ...
    def create_multi_shoes(self, db: Session, shoes: List[ShoeCreateSchema],current_user_role_permission: UserRolePermissionSchema) -> List[ShoeOutSchema]:
        ...
        shoes_saved = []
        for shoe in shoes:
            shoes_saved.append(self.create_shoe(db=db, shoe=shoe, current_user_role_permission=current_user_role_permission))
        return shoes_saved
    ...
    def get_shoe_by_id(self, db: Session, shoe_id: uuid.UUID,current_user_role_permission: UserRolePermissionSchema) -> Optional[ShoeOutSchema]:
        ...
        try:
            ...
            shoe_out = ShoeOutSchema(
                ...
                size=SizeCreateSchema(
                    size_number=shoe_found.size.size_number
                ),
    ...
    def delete_shoe(self, db: Session, shoe_id: uuid.UUID,current_user_role_permission: UserRolePermissionSchema) -> Optional[ShoeOutSchema]:
        ...
        try:
            ...
            return JSONResponse(
                status_code=200,
                content=f"Shoe {shoe_removed.id} removed"
            )
        except:
            db.rollback()
            logger.exception(
