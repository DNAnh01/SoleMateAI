...
class MessageServiceImpl(MessageService):
    ...
    def create(self, message: str, conversation_id: uuid.UUID, client_ip: str, db: Session,current_user_role_permission: UserRolePermissionSchema) -> MessageInDBSchema:
        if 'create_message' and 'read_chatbot' not in current_user_role_permission.u_list_permission_name:
            logger.exception(
        ...
        try:
            ...
            conversation = self.__crud_conversation.get(
                db=db,
    ...
    def handle_message(
            self, db: Session, chatbot_id: str, conversation_id: str):
        try:
            temp_knowledge_base = []
            messages = self.__crud_message.get_multi(
                db=db, 
                filter_param={"conversation_id": conversation_id}
            )
            knowledge_bases = self.__crud_knowledge_base.get_multi(
                db=db, 
                filter_param={"chatbot_id": chatbot_id}
            )
            chatbot_found = self.__crud_chatbot.get(
                db=db,
                id=chatbot_id
            )
            ...
            for message_ in messages:
                temp_knowledge_base.append(
                    {
                        'role': 'user' if message_.sender_type == 'guest' else 'assistant',
            ...
            return "An error occurred while processing your request. Please try again later.", chatbot_id
