...
from backend.schemas.order_item_schema import OrderItemOutSchema, OrderItemInDBSchema
...
from backend.schemas.cart_schema import CartInDBSchema
...
from backend.schemas.shoe_schema import ShoeInDBSchema, ShoeUpdateSchema
...
class OrderServiceImpl(OrderService):
    ...
    def create_order(
        ...
        try:
            cart_found = self.__crud_cart.get_one_by(
                db=db,
                filter={"user_id": current_user_role_permission.u_id}
            )
            ...
            for cart_item in cart_found.cart_items:
                ...
                shoe_found = self.__crud_shoe.get(
                    db=db,
                    id=cart_item.shoe_id
                )
                ...
                updated_shoe = self.__crud_shoe.update_one_by(
                    ...
                    obj_in=ShoeUpdateSchema(
                        quantity_in_stock=shoe_found.quantity_in_stock - cart_item.quantity,
                        updated_at=datetime.now()
                    )
                )
    ...
    def get_history_order(
        ...
        try:
            ...
            for order in history_orders:
                ...
                for order_item in order_items_db:
                    if order_item.order_id == order.id:
                        ...
                        shoe = self.__crud_shoe.get_one_ignore_deleted_and_inactive(db=db, filter={"id": shoe_id})
                        brand = self.__crud_brand.get_one_ignore_deleted_and_inactive(db=db, filter={"id": shoe.brand_id})
                        size = self.__crud_size.get_one_ignore_deleted_and_inactive(db=db, filter={"id": shoe.size_id})
                        color = self.__crud_color.get_one_ignore_deleted_and_inactive(db=db, filter={"id": shoe.color_id})
                        order_items.append(
                            OrderItemOutSchema(
                                shoe=ShoeOutSchema(
            ...
            logger.exception(f"Exception in {__name__}.{self.__class__.__name__}.get_history_order: {str(e)}")
            return JSONResponse(
                status_code=500, 
                content={
                    "status": 500,
    ...
    def get_order_by_id(
        ...
        try:
            ...
            logger.exception(f"Exception in {__name__}.{self.__class__.__name__}.get_order_by_id: {str(e)}")
            return JSONResponse(
                status_code=500, 
                content={
                    "status": 500,       
        ...
        try:
            ...
            for order_item in order_found.order_items:
                ...
                updated_shoe = self.__crud_shoe.update_one_by(
                    ...
                    obj_in=ShoeUpdateSchema(
                        quantity_in_stock=shoe_found.quantity_in_stock + order_item.quantity,
                        updated_at=datetime.now()
                    )
                )
                if updated_shoe is None:
            ...
            logger.exception(f"Exception in {__name__}.{self.__class__.__name__}.cancel_order: {str(e)}")
            return JSONResponse(
                status_code=500, 
                content={
                    "status": 500,
    ...
    def paid_order(
        ...
        try:
            ...
            logger.exception(f"Exception in {__name__}.{self.__class__.__name__}.paid_order: {str(e)}")
            return JSONResponse(
                status_code=500, 
                content={
                    "status": 500,       
