...
def get_all(
    ...
    return chatbot_service.get_all(db=db, current_user_role_permission=current_user_role_permission)

@router.get("/id={chatbot_id}", status_code=status.HTTP_200_OK, response_model=Optional[ChatbotInDBSchema])
def get_one(
    chatbot_id: str,
    current_user_role_permission: UserRolePermissionSchema = Depends(oauth2.get_current_user_role_permission),
    db: Session = Depends(deps.get_db)
...
@router.patch("/id={chatbot_id}", status_code=status.HTTP_200_OK, response_model=ChatbotInDBSchema)
def update(
    chatbot_id: str,
...
@router.delete("/id={chatbot_id}", status_code=status.HTTP_200_OK, response_model=ChatbotInDBSchema)
def delete(
    chatbot_id: str,
    current_user_role_permission: UserRolePermissionSchema = Depends(oauth2.get_current_user_role_permission),
    db: Session = Depends(deps.get_db)
