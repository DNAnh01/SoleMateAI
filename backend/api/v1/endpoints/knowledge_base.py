...
@router.get("/chatbot-id={chatbot_id}/get-all", status_code=status.HTTP_200_OK)
def get_all(
    chatbot_id: str,
    current_user_role_permission: UserRolePermissionSchema = Depends(oauth2.get_current_user_role_permission),
    db: Session = Depends(deps.get_db)
) -> Optional[List[KnowledgeBaseCreateSchema]]:
...
@router.delete("/chatbot-id={chatbot_id}/knowledge-base-id={knowledge_base_id}", status_code=status.HTTP_200_OK)
def delete(
    chatbot_id: str,
