...
from backend.schemas.auth_schema import EmailSchema
from backend.schemas.user_schema import  UserInDBSchema, UserUpdateSchema, UserOutSchema
...
# ADMIN
@router.get("/user-id={user_id}", status_code=status.HTTP_200_OK, response_model=Optional[UserOutSchema])
def get_user_by_id(
    user_id: str,
...
# USER
@router.get("/profile", status_code=status.HTTP_200_OK, response_model=Optional[UserOutSchema])
...
# ADMIN
@router.get("/get-all", status_code=status.HTTP_200_OK, response_model=Optional[List[UserOutSchema]])
def get_all_users(
...
# USER
@router.patch("/profile", status_code=status.HTTP_200_OK, response_model=Optional[UserOutSchema])
def update_user(
